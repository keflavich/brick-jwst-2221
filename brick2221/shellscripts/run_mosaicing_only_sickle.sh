...
submit_residual_mosaic_only() {
    ...
    for kind in basic iterative; do
        for module_name in "${modules[@]}"; do
            local pattern="${basepath}/${filter}/pipeline/jw0${proposal_id}-o${field}_t001_nircam_clear-${filter,,}-${module_name}_visit*_vgroup*_exp*${bgsub_suffix}_daophot_${kind}_residual.fits"
    ...
    sbatch --job-name=webb-mosaic-sickle-${filter}-${module}${bgsub_tag} \
        --output=${logdir}/webb-mosaic-sickle-${filter}-${module}${bgsub_tag}_%j.log \
