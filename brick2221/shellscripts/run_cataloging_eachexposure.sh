...
for filter in F410M F405N F466N; do
    for module in nrca nrcb; do
        for dao in "${daoloop[@]}"; do
            sbatch --array=0-23 --job-name=webb-cat-${filter}-${module}-eachexp --output=${logdir}/webb-cat-${filter}-${module}-eachexp_%j-%A_%a.log  --account=astronomy-dept --qos=astronomy-dept-b --ntasks=2 --nodes=1 --mem=${mem} --time=96:00:00 --wrap "/blue/adamginsburg/adamginsburg/miniconda3/envs/python313/bin/python /blue/adamginsburg/adamginsburg/jwst/brick/analysis/crowdsource_catalogs_long.py --filternames=${filter} --modules=${module} --each-exposure ${dao}"
        done
    done
done
